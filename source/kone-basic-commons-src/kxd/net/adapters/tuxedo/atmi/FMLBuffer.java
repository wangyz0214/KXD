...
public class FMLBuffer extends Buffer {
	int bufSize;
	...
	public void reset() throws TuxedoException {
		dispose();
		handle = TuxedoAtmi.tpalloc("FML".getBytes(), null, bufSize);
		if (handle <= 0)
			throw new TuxedoException(0, "分配缓冲区失败");
		if (TuxedoAtmi.Finit(handle) == -1) {
			dispose();
			throw new TuxedoException(0, "初始化FML缓冲区失败");
		}
	}

	public FMLBuffer(int bufSize) throws TuxedoException {
	...
	public int read(int row, int col, byte[] data, int length)
		...
		checkDisposed();
		int ret = (int) TuxedoAtmi.Fget(handle, row, col, data, length);
		if (ret <= 0)
	...
	public String read(int row, int col, int length) throws TuxedoException {
		...
		int len = (int) TuxedoAtmi.Fget(handle, row, col, r, length);
		if (len <= 0)
			throw new TuxedoException(0, "读FML数据失败");
		else if (len <= 1)
			return "";
	...
	public int readInt(int row, int col) throws TuxedoException {
		checkDisposed();
		byte[] r = new byte[5];
		int len = (int) TuxedoAtmi.Fget(handle, row, col, r, 5);
		if (len < 4)
			throw new TuxedoException(0, "读FML数据失败");
		else
	...
	public short readShort(int row, int col) throws TuxedoException {
		checkDisposed();
		byte[] r = new byte[3];
		int len = (int) TuxedoAtmi.Fget(handle, row, col, r, 3);
		if (len < 2)
			throw new TuxedoException(0, "读FML数据失败");
		else
	...
	public long readLong(int row, int col) throws TuxedoException {
		checkDisposed();
		byte[] r = new byte[9];
		int len = (int) TuxedoAtmi.Fget(handle, row, col, r, 9);
		if (len < 8)
			throw new TuxedoException(0, "读FML数据失败");
		else
	...
	public int next(FMLID row, FMLID col, byte[] data, int length)
		...
		rowcol[0] = row.id;
		rowcol[1] = col.id;
		int ret = (int) TuxedoAtmi.Fnext(handle, rowcol, data, length);
		if (ret <= 0)
			return ret;
		else {
			row.id = rowcol[0];
	...
	public void writeDouble(int row, int col, double data)
			throws TuxedoException {
		checkDisposed();
		if (!TuxedoAtmi.Fset(handle, row, col, DataUnit.doubleToBytes(data)))
			throw new TuxedoException(0, "写FML数据失败");
	}

	public void writeFloat(int row, int col, float data) throws TuxedoException {
		checkDisposed();
		if (!TuxedoAtmi.Fset(handle, row, col, DataUnit.floatToBytes(data)))
			throw new TuxedoException(0, "写FML数据失败");
	}

	public int occur(int row) {
		return TuxedoAtmi.Foccur(handle, row);
	}

	public void print() throws TuxedoException {
		checkDisposed();
		TuxedoAtmi.Fprint(handle);
	}
}
